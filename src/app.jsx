...
import {
  ...
  BayPanel, AdminSettings, AdminServiceManager,
} from './admin.jsx';
...
function MobileView({ services, settings, setSettings, setTweak, t, blocked }) {
  ...
  const startBooking = () => { setBookingInit([]); setTab('booking'); };
  const startBookingWith = (ids) => { setBookingInit(ids); setTab('booking'); };
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg)', overflow: 'hidden', position: 'relative' }}>
      ...
      <div className="phone-scroll" style={{ flex: 1, overflowY: 'auto', background: 'var(--bg)' }}>
        {screen}
      </div>
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakSection label="Demo">
          <TweakButton label="Jump to booking" onClick={() => { setBookingInit(['svc-oil', 'svc-tire']); setTab('booking'); }} />
        </TweakSection>
      </TweaksPanel>
    </div>
  );
...
function AdminView({ services, appointments, setAppointments, settings, setSettings, setTweak, t }) {
    ...
    switch (page) {
      ...
      case 'feed': return (
        <div style={{ padding: 20 }}>
          <div className="mono" style={{ fontSize: 10, color: 'var(--mute)' }}>[ APPOINTMENTS ]</div>
          <div className="display" style={{ fontSize: 28, fontWeight: 700, marginTop: 4, marginBottom: 16, color: 'var(--text)' }}>All appointments</div>
          <AppointmentFeed appointments={appointments} services={services} updateAppointment={updateAppointment} />
        </div>
      );
      case 'bays': return (
        <div style={{ padding: 20 }}>
          <div className="mono" style={{ fontSize: 10, color: 'var(--mute)' }}>[ BAYS ]</div>
          <div className="display" style={{ fontSize: 28, fontWeight: 700, marginTop: 4, marginBottom: 16, color: 'var(--text)' }}>Shop floor</div>
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', background: 'var(--bg-2)', overflow: 'hidden' }}>
  ...
  return (
    <div className={themeClass} style={{ color: 'var(--text)', fontFamily: 'var(--font-body)' }}>
      <Routes>
        ...
        <Route path="/:tab" element={<MobileView {...sharedProps} blocked={blocked} />} />
      </Routes>
    </div>
  );
}
